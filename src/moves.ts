...
let flag = false;
let currentPiece : Piece | string = " ";
let prev_row : number | null = null;
let prev_col : number | null = null;

export default function move(this: HTMLElement, e : Event){

    const target = e.target as HTMLDivElement;

    let row_str: string | null = target?.getAttribute('row');
    let col_str: string | null = target?.getAttribute('col');
